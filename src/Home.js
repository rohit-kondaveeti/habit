...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("habits")) {
      ...
      const data = [];
      for (let habit of JSON.parse(localStorage.getItem("habits"))) {
        let u = 0;
        let d = 0;
        let n = 0;
        for (let d of habit.date) {
          if (d.status == 0) n++;
          else if (d.status == 1) d++;
          else if (d.status == 2) u++;
          var h = d.id;
        }
        d = +7 - (+n + +u);
        data[habit.id] = { d: d, n: n, u: u };
      }
      setCount(data);
      setActive(true);
    }
              ...
              <div className="habits_data">
                {habits.map((habit, i) => (
                  <div key={i}>
                    ...
                    <p className="details">
                      <span>Done: {count[habit.id].d}/7</span>
                      <span>None: {count[habit.id].n}/7</span>
                      <span>Undone: {count[habit.id].u}/7</span>
                    </p>
                  </div>
                ))}
              </div>
